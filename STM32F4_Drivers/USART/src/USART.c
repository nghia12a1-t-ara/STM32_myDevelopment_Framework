 ...
 */
void USART_PeripheralControl(USART_Types *pUSARTx, uint8 Cmd)
{
	if(Cmd == ENABLE)
	{
		pUSARTx->CR1 |= (1 << 13);
   ...
   //Loop over until "Len" number of bytes are transferred
	for(uint32 i = 0 ; i < Len; i++)
	{
		...
		if( pUSARTHandle->USART_Config.USART_WordLength == USART_WORDLEN_9BITS )
		{
			...
			pUSARTHandle->pUSARTx->DR = (*pdata & (uint16)0x01FF);
		...
		else
		{
			...
			pUSARTHandle->pUSARTx->DR = (*pTxBuffer  & (uint8)0xFF);
	...
	for(uint32 i = 0 ; i < Len; i++)
	{
		...
		if ( pUSARTHandle->USART_Config.USART_WordLength == USART_WORDLEN_9BITS )
		{
			...
			if ( pUSARTHandle->USART_Config.USART_ParityControl == USART_PARITY_DISABLE )
			{
				...
				*((uint16*) pRxBuffer) = (pUSARTHandle->pUSARTx->DR  & (uint16)0x01FF);
			...
			else
			{
				 ...
				 *pRxBuffer = (pUSARTHandle->pUSARTx->DR  & (uint8)0xFF);
				 pRxBuffer++;
			}
		}
		else
		{
			...
			if ( pUSARTHandle->USART_Config.USART_ParityControl == USART_PARITY_DISABLE )
			{
				...
				//read 8 bits from DR
				 *pRxBuffer = (uint8) (pUSARTHandle->pUSARTx->DR  & (uint8)0xFF);
			}
			...
			else
			{
				...
				*pRxBuffer = (uint8) (pUSARTHandle->pUSARTx->DR  & (uint8)0x7F);
			}
...
{
	uint8 txstate = pUSARTHandle->TxBusyState;
...
{
	uint8 rxstate = pUSARTHandle->RxBusyState;
	...
	if ( temp1 && temp2 )
	{
		...
		if ( pUSARTHandle->TxBusyState == USART_BUSY_IN_TX )
		{
			...
			if ( pUSARTHandle->TxLen > 0 )
			{
				...
				{
					//if 9BIT load the DR with 2bytes masking  the bits other than first 9 bits
					pdata = (uint16*) pUSARTHandle->pTxBuffer;
					pUSARTHandle->pUSARTx->DR = (*pdata & (uint16)0x01FF);
				...
				{
					//This is 8bit data transfer
					pUSARTHandle->pUSARTx->DR = (*pUSARTHandle->pTxBuffer  & (uint8)0xFF);
			...
			if ( pUSARTHandle->RxLen > 0 )
			{
				...
				if ( pUSARTHandle->USART_Config.USART_WordLength == USART_WORDLEN_9BITS )
				{
					...
					if ( pUSARTHandle->USART_Config.USART_ParityControl == USART_PARITY_DISABLE )
					{
						...
						//read only first 9 bits so mask the DR with 0x01FF
						*((uint16*) pUSARTHandle->pRxBuffer) = (pUSARTHandle->pUSARTx->DR  & (uint16)0x01FF);
					...
					{
						//Parity is used, so 8bits will be of user data and 1 bit is parity
						 *pUSARTHandle->pRxBuffer = (pUSARTHandle->pUSARTx->DR  & (uint8)0xFF);
						 pUSARTHandle->pRxBuffer++;
				...
				else
				{
					...
					if ( pUSARTHandle->USART_Config.USART_ParityControl == USART_PARITY_DISABLE )
					{
						...
						//read 8 bits from DR
						 *pUSARTHandle->pRxBuffer = (uint8) (pUSARTHandle->pUSARTx->DR  & (uint8)0xFF);
					}
					else
					{
						//Parity is used, so , 7 bits will be of user data and 1 bit is parity
						...
						//read only 7 bits , hence mask the DR with 0X7F
						*pUSARTHandle->pRxBuffer = (uint8) (pUSARTHandle->pUSARTx->DR  & (uint8)0x7F);
					}
 ...
 */
__weak void USART_ApplicationEventCallback(USART_Handle_t *pUSARTHandle, uint8 event)
{
